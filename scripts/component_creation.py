...
for results_dir in results:
    ...
    component_local_creation = float((x for x in components_data['benchmarks']
        if x['name'] == 'simple_component_local/real_time').next()['real_time'])

    component_remote_creation = float((x for x in components_data['benchmarks']
        if x['name'] == 'simple_component_remote/real_time').next()['real_time'])

    print(component_local_creation)
    print(component_remote_creation)

    times = np.array([component_local_creation, component_remote_creation])
...
fname = '%s/component_creation.pgf' % os.path.join(results_base, '../figures/')
plt.savefig(fname)
